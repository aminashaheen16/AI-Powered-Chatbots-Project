...
st.markdown("""
    ...
    * { font-family: 'Outfit', sans-serif; }
    ...
    .card { background-color: rgba(30, 41, 59, 0.5); padding: 15px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.05); text-align: center; transition: 0.3s; }
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<p style='color:#888; font-size:0.8rem; margin-bottom:5px;'>ENGINE SETTINGS</p>", unsafe_allow_html=True)
    bot_type = st.selectbox("Active Protocol", ["Inventory Bot (SQL)", "Knowledge Graph Bot (Neo4j)"], label_visibility="collapsed")
    st.session_state.sessions[st.session_state.current_session_id]["bot_type"] = bot_type
    
    st.markdown("---")
    st.markdown("<p style='color:#888; font-size:0.8rem; margin-bottom:5px;'>HISTORY</p>", unsafe_allow_html=True)
...
# --- Main App ---
st.markdown("<div style='display:flex; justify-content:space-between; margin-bottom:20px;'><h3>Insights Dashboard</h3><p>🟢 System Optimized (Groq)</p></div>", unsafe_allow_html=True)
